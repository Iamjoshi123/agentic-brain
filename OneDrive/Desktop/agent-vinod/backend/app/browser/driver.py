...
class BrowserDriver(ABC):
    ...
    @abstractmethod
    async def start(self, headless: bool = True, record_video_dir: Optional[str] = None) -> None:
    ...
    async def get_recording_artifact(self) -> Optional[str]:
        """Return the finalized recording path, if one exists."""
        return None
    ...
    async def close(self) -> None:
        try:
            ...
            if video is not None:
                try:
                    self._recorded_video_path = await video.path()
                except Exception as exc:
                    logger.debug("Video path unavailable: %s", exc)
            if self._playwright:
...
class FakeBrowserDriver(BrowserDriver):
    ...
    async def start(self, headless: bool = True, record_video_dir: Optional[str] = None) -> None:
        self._started = True
        self._title = "Fake Browser"
        self._recording_path = str(Path(record_video_dir) / "fake-session.webm") if record_video_dir else None
    ...
    async def get_recording_artifact(self) -> Optional[str]:
        return getattr(self, "_recording_path", None)
