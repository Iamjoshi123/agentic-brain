...
class LiveDemoRuntime:
    ...
    async def start(self, db: Session, session: DemoSession, workspace: Workspace) -> LiveStartResult:
        ...
        product_config = db.exec(
            select(ProductConfig).where(ProductConfig.workspace_id == workspace.id)
        ).first()
        if self._meeting_id:
        ...
        self._greeting_text = (
            product_config.greeting_template
            if product_config and product_config.greeting_template
            else build_greeting_text(
    ...
    async def speak_intro_greeting(self) -> None:
        if self._greeting_sent or not self._greeting_text:
            return
        try:
            await asyncio.wait_for(self._media_ready.wait(), timeout=8)
    ...
    def _launch_media_start(self, driver: object, agent_contract: LiveKitParticipantContract) -> None:
        ...
        for attempt, delay_seconds in enumerate(delays, start=1):
            ...
            if attempt > 1:
                attempt_contract = create_livekit_participant(
                    ...
                    identity=f"agent-{self.session_id}-{uuid.uuid4().hex[:8]}",
                    can_publish=True,
                    can_subscribe=True,
                )
                logger.info(
            ...
            try:
                await self._media.start(
                    driver,
                    attempt_contract,
                    self.handle_buyer_transcript,
                    self.handle_buyer_activity,
                )
                ...
                await self._publish_event(
                    _event(
                        "browser_stage_state",
                        state="attaching",
                        detail="Publishing the browser video track to the room.",
                    )
                )
