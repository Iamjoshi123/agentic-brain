...
def create_session(data: SessionCreate, db: Session = Depends(get_session)):
    ...
    product_config = db.exec(
        select(ProductConfig).where(ProductConfig.workspace_id == ws.id)
    ).first()
    agent_name = product_config.agent_name if product_config else "your AI assistant"
    greeting = product_config.greeting_template if product_config else None
    ...
    welcome = SessionMessage(
        ...
        content=(
            greeting
            or (
                f"Welcome to the {ws.name} demo! I'm {agent_name}. "
...
async def end_session(session_id: str, db: Session = Depends(get_session)):
    ...
    if recording is not None and session.started_at and session.ended_at:
