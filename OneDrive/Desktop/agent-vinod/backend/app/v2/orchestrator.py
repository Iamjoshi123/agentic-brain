    ...
    async def handle_turn(
        ...
        *,
        ...
        if intent.policy_decision == "escalate":
            return MeetingTurn(
                response_text=(
                    "That part is best handled by a human commercial conversation. "
                    "I can still keep walking you through the product and show the relevant workflow while sales follows up."
                ),
                stage="handoff",
...
async def _generate_answer_reply(
    *,
    ...
    prompt_parts.append(
        ...
        "Walk the buyer through the product in the order it appears on screen, and explain only what is being shown now or next."
    )
    prompt_parts.append(f"Respond in {language_name(preferred_language)}.")
    if realtime:
        prompt_parts.append(
            "This response will be spoken live. Keep it to one or two short sentences, sound empathetic and enthusiastic, and make it feel like a guided walkthrough."
        )

    system = (
        "You are a high-signal demo agent with an empathetic, enthusiastic tone. "
        "Answer only from the supplied product context. "
        "If the buyer wants a walkthrough, show while telling: mention the concrete UI area you are showing now and the next thing you will open."
    )
    if realtime:
...
def _compose_direct_reply(
    *,
    ...
    if action_plan.strategy == "clarify":
        return (
            "Absolutely, I'll start with the most relevant part of the product and walk you through it live."
            if realtime
            else "Absolutely, I'll start with the most relevant part of the product and walk you through it in the live product."
        )

    if action_plan.strategy == "stagehand_first":
        target = _action_target_label(action_plan.candidate, observation, action_plan.stagehand_instruction)
        if realtime:
            return f"Great, I'm opening {target} in {product_name} now and I'll walk you through what matters as it appears."
        return f"Great, I'll open {target} in the live {product_name} experience now and walk you through what matters as it appears."

    if action_plan.strategy == "recipe_fallback" and recipe is not None:
        if realtime:
            return f"Great, I'll walk you through {recipe.name} live and explain each step while I show it."
        return f"Great, I'll use the {recipe.name} walkthrough in the live product and explain each step while I show it."
...
def _fallback_answer_reply(
    ...
    if observation.screen_summary and observation.active_module:
        return (
            f"We're on {observation.active_module} right now. "
            f"{observation.screen_summary} I'll keep walking you through it step by step."
        )
    ...
    if recipe is not None:
        return f"I'll use the {recipe.name} walkthrough and explain the product as I show each step."
    if retrieval.context_text:
        return f"Based on the product context, {retrieval.context_text[:220]} I'll keep the walkthrough moving and explain it as I show it."
    return "I'll lead the walkthrough from here and explain the product live as I show it."
