...
export default function AdminSessionsClient({
  ...
  const filteredSessions = useMemo(() => {
    ...
    return sessions.filter((session) =>
      `${session.buyer_name || ""} ${session.product_name || ""}`.toLowerCase().includes(term),
    );
  }, [search, sessions]);
        ...
        <aside className="rounded-[24px] border border-[var(--border-subtle)] bg-[rgba(255,255,255,0.92)] px-4 py-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-[1.35rem] font-medium tracking-[-0.04em] text-[var(--text-primary)]">
                All products
              </h2>
            </div>
              ...
              filteredSessions.map((session) => (
                ...
                >
                  <div className="min-w-0">
                    <p
                      className="admin-truncate text-sm text-[var(--text-primary)]"
                      title={session.buyer_name || "Anonymous prospect"}
                    >
                      {session.buyer_name || "Anonymous prospect"}
                    </p>
                    <p className="mt-1 text-xs text-[var(--text-tertiary)]">{formatDate(session.started_at)}</p>
                  </div>
                  <p
                    className="admin-truncate min-w-0 text-sm text-[var(--text-secondary)]"
                    title={session.product_name || "Unknown product"}
                  >
            ...
            <>
              <div className="rounded-[24px] border border-[var(--border-subtle)] bg-[rgba(255,255,255,0.92)] px-5 py-5">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <h2
                      className="admin-wrap mt-2 text-[1.5rem] font-medium tracking-[-0.04em] text-[var(--text-primary)]"
                      title={detail.session?.buyer_name || "Anonymous prospect"}
                    >
                      {detail.session?.buyer_name || "Anonymous prospect"}
                    </h2>
                    <p className="admin-wrap mt-2 text-sm text-[var(--text-secondary)]">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <span className="badge">
                      {detail.summary?.total_messages ?? detail.messages?.length ?? 0} messages
                    </span>
                    <span className="badge">
                      {detail.summary?.total_actions ?? detail.actions?.length ?? 0} actions
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-[24px] border border-[var(--border-subtle)] bg-[rgba(255,255,255,0.92)] px-5 py-5">
                ...
                <div className="mt-5 space-y-3">
                  {detail.messages?.map((message: any) => (
                    <div
                      key={message.id}
                      className="rounded-[18px] border border-[var(--border-subtle)] bg-[var(--surface-muted)] px-4 py-3"
                    >
                      <p className="text-xs uppercase tracking-[0.14em] text-[var(--text-tertiary)]">{message.role}</p>
                      <p className="admin-wrap mt-2 text-sm leading-6 text-[var(--text-primary)]">{message.content}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                <div className="rounded-[24px] border border-[var(--border-subtle)] bg-[rgba(255,255,255,0.92)] px-5 py-5">
                  ...
                  <p className="admin-wrap mt-3 text-sm leading-7 text-[var(--text-secondary)]">
                  ...
                  <div className="mt-4 grid gap-4 md:grid-cols-3">
                    <div>
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        {parseList(detail.summary?.top_questions).map((item) => (
                    ...
                    <div>
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        {parseList(detail.summary?.features_interest).map((item) => (
                          <span key={item} className="badge" title={item}>
                            {item}
                          </span>
                        ))}
                    ...
                    <div>
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        {parseList(detail.summary?.objections).map((item) => (
                          <span key={item} className="badge" title={item}>
                            {item}
                          </span>
                        ))}
                ...
                <div className="rounded-[24px] border border-[var(--border-subtle)] bg-[rgba(255,255,255,0.92)] px-5 py-5">
                  ...
                  <div className="mt-4 space-y-3">
                    {detail.citations_used?.length ? (
                      detail.citations_used.map((citation: any) => (
                        <div
                          key={citation.document_id}
                          className="rounded-[18px] border border-[var(--border-subtle)] bg-[var(--surface-muted)] px-4 py-3"
                        >
                          <p className="admin-wrap text-sm text-[var(--text-primary)]">{citation.title}</p>
                          <p className="mt-1 text-xs uppercase tracking-[0.14em] text-[var(--text-tertiary)]">
                            {citation.source_type}
                          </p>
                          <p className="admin-wrap mt-2 text-sm leading-6 text-[var(--text-secondary)]">
                            {citation.excerpt}
                          </p>
                        </div>
                      ))
