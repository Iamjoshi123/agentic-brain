...
type BrowserRenderMode = "video" | "canvas";
...
export default function MeetingPageV2() {
  ...
  const [browserRenderMode, setBrowserRenderMode] = useState<BrowserRenderMode>("canvas");
  ...
  const browserStageHealthyRef = useRef(false);
  ...
  async function disconnectLiveSession() {
    ...
    browserStageHealthyRef.current = false;
    setBrowserRenderMode("video");
    setAudioReady(false);
    setMicReady(false);
    setLiveInfo(null);
  }
  ...
  function renderBrowserVideoToCanvas() {
    ...
    const width = Math.max(video.videoWidth || 1280, 1);
    const height = Math.max(video.videoHeight || 720, 1);
    ...
    if (videoAverage >= 6 && browserRenderMode !== "canvas") {
      console.info("[browser-stage] switching render mode to canvas", {
        width,
        height,
        averageBrightness: Number(videoAverage.toFixed(2)),
      });
      setBrowserRenderMode("canvas");
    }
    if (videoAverage >= 6 && browserStageState !== "live") {
      setBrowserTrackReady(true);
      setBrowserStageState("live");
      browserStageHealthyRef.current = true;
  ...
  function handleLiveEvent(event: any) {
    ...
    if (event.type === "browser_stage_state") {
      ...
      if (browserStageHealthyRef.current && state === "attaching") {
        console.info("[browser-stage] ignoring late attaching state because browser stage is already healthy");
        return;
      }
      setBrowserStageState(state);
      browserStageHealthyRef.current = state === "live";
      if (state !== "live") {
        setBrowserTrackReady(false);
      }
      if (event.detail) {
        ...
        if (element instanceof HTMLVideoElement) {
          element.onloadeddata = () => {
            ...
            if (element.videoWidth > 0 && element.videoHeight > 0) {
              ...
              setBrowserStageState("attaching");
              browserStageHealthyRef.current = false;
              setRuntimeNote("Live browser video attached.");
              setLiveError(null);
            } else {
              setBrowserStageState("attaching");
              setRuntimeNote("Browser track attached. Validating live frames.");
            }
          ...
          element.onplaying = async () => {
            ...
            if (element.videoWidth > 0 && element.videoHeight > 0) {
              setBrowserRenderMode("canvas");
              setBrowserTrackReady(false);
              setBrowserStageState(average >= 6 ? "live" : "attaching");
              browserStageHealthyRef.current = average >= 6;
            ...
            <div className="relative min-h-0 flex-1 overflow-hidden rounded-[30px] border border-[var(--demo-border)] bg-[rgba(255,255,255,0.02)] p-2 shadow-[var(--demo-shadow)]">
              ...
              <div className="relative h-full overflow-hidden rounded-[24px] border border-[var(--demo-border)] bg-[#030304]">
                ...
                <canvas
                  ...
                  data-testid="browser-stage-canvas"
                  className={`absolute inset-0 z-[1] h-full w-full ${
                    browserRenderMode === "canvas" ? "opacity-100" : "opacity-0"
                  }`}
                />
                {effectiveBrowserStageState !== "live" && (
